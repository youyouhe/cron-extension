...
[Oh My Pi (omp)](https://github.com/oh-my-pi) 会话内定时任务插件：让 agent 在对话中建立定时任务，到点后自动驱动当前会话执行。
...
重启 omp 会话生效。加载成功后输入 `/cron` 应显示「当前没有定时任务。」；删除文件或软链即卸载。
